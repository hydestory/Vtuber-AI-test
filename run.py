...
openai.api_key ="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"
...
def translate_text(text):
    # result_ch will act as subtitle for the viewer
    result_ch = translate_google(text, "EN", "zh-TW")
    print("中文回復: " + result_ch)
    # result_jp will be the string to be converted to audio
    result_jp = translate_google(text, "EN", "JA")
    print("JP Answer: " + result_jp)
    speech_text(result_jp, result_ch)

def speech_text(result_jp, result_ch):
    ...
    request = requests.post(f'http://0e10-34-171-158-222.ngrok.io/audio_query?{params_encoded}')
    params_encoded = urllib.parse.urlencode({'speaker': 20, 'enable_interrogative_upspeak': True})
    request = requests.post(f'http://0e10-34-171-158-222.ngrok.io/synthesis?{params_encoded}', json=request.json())
